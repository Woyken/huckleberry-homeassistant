...
async def test_services(hass: HomeAssistant, mock_huckleberry_api):
    ...
    mock_huckleberry_api.start_sleep.assert_awaited_with("test_child_uid")

    # Test pause_sleep
    await hass.services.async_call(
        DOMAIN, "pause_sleep", {"device_id": device.id}, blocking=True
    )
    mock_huckleberry_api.pause_sleep.assert_awaited_with("test_child_uid")

    # Test resume_sleep
    await hass.services.async_call(
        DOMAIN, "resume_sleep", {"device_id": device.id}, blocking=True
    )
    mock_huckleberry_api.resume_sleep.assert_awaited_with("test_child_uid")

    # Test cancel_sleep
    await hass.services.async_call(
        DOMAIN, "cancel_sleep", {"device_id": device.id}, blocking=True
    )
    mock_huckleberry_api.cancel_sleep.assert_awaited_with("test_child_uid")

    # Test complete_sleep
    await hass.services.async_call(
        DOMAIN, "complete_sleep", {"device_id": device.id}, blocking=True
    )
    mock_huckleberry_api.complete_sleep.assert_awaited_with("test_child_uid")

    # Test start_feeding (left)
    await hass.services.async_call(
        DOMAIN, "start_feeding", {"device_id": device.id, "side": "left"}, blocking=True
    )
    mock_huckleberry_api.start_nursing.assert_awaited_with("test_child_uid", "left")

    # Test start_feeding (right)
    await hass.services.async_call(
        DOMAIN, "start_feeding", {"device_id": device.id, "side": "right"}, blocking=True
    )
    mock_huckleberry_api.start_nursing.assert_awaited_with("test_child_uid", "right")

    # Test pause_feeding
    await hass.services.async_call(
        DOMAIN, "pause_feeding", {"device_id": device.id}, blocking=True
    )
    mock_huckleberry_api.pause_nursing.assert_awaited_with("test_child_uid")

    # Test resume_feeding
    await hass.services.async_call(
        DOMAIN, "resume_feeding", {"device_id": device.id}, blocking=True
    )
    mock_huckleberry_api.resume_nursing.assert_awaited_with("test_child_uid", None)

    # Test switch_feeding_side
    await hass.services.async_call(
        DOMAIN, "switch_feeding_side", {"device_id": device.id}, blocking=True
    )
    mock_huckleberry_api.switch_nursing_side.assert_awaited_with("test_child_uid")

    # Test cancel_feeding
    await hass.services.async_call(
        DOMAIN, "cancel_feeding", {"device_id": device.id}, blocking=True
    )
    mock_huckleberry_api.cancel_nursing.assert_awaited_with("test_child_uid")

    # Test complete_feeding
    await hass.services.async_call(
        DOMAIN, "complete_feeding", {"device_id": device.id}, blocking=True
    )
    mock_huckleberry_api.complete_nursing.assert_awaited_with("test_child_uid")

    # Test log_diaper_pee
    await hass.services.async_call(
        DOMAIN, "log_diaper_pee", {"device_id": device.id, "pee_amount": "medium"}, blocking=True
    )
    mock_huckleberry_api.log_diaper.assert_awaited_with(
    ...
    mock_huckleberry_api.log_growth.assert_awaited_with(
    ...
    mock_huckleberry_api.log_bottle.assert_awaited_with(
...
async def test_service_explicit_child_uid(hass: HomeAssistant, mock_huckleberry_api):
    ...
    mock_huckleberry_api.start_sleep.assert_awaited_with("explicit_child_uid")
