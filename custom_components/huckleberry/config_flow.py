    ...
    async def async_step_user(
        ...
        if user_input is not None:
            try:
                ...
                children = user.childList if user is not None else []

                if not children:
                    errors["base"] = "no_children"
                else:
                    # Create entry
                    await self.async_set_unique_id(api.user_uid or user_input[CONF_EMAIL])
                ...
                error_code = "invalid_auth" if err.status == 400 else "cannot_connect"
                ...
                _LOGGER.exception("Client error during authentication")
                errors["base"] = "cannot_connect"
                description_placeholders["error_details"] = f"\n\n**Error:** {err}"
